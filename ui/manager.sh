...
if [[ ${bypass_user_selection} -eq 0 ]]; then
    ...
    echo '27 - Deploy with HTTPS (Apache/Nginx)'
...
configure_nginx(){
    echo_color "=== Nginx Configuration for HTTPS + WebSocket ===" ${color_yellow}
    echo ""
    
    # Check if Nginx is installed
    if ! command -v nginx &> /dev/null; then
        echo_error "Nginx is not installed"
        echo_info "Install with: sudo apt update && sudo apt install nginx"
        exit 1
    fi
    
    echo_info "Detected Nginx installation"
    echo ""
    ...
    if [ ! -d "$dist_path" ]; then
        ...
        if [ ! -d "$dist_path/icons" ] && [ -d "public/icons" ]; then
            echo_warning "Icons directory not found in dist/ - copying from public/"
            sudo cp -r public/icons "$dist_path/"
            sudo chown -R www-data:www-data "$dist_path/icons"
            sudo chmod -R 755 "$dist_path/icons"
            echo_success "Icons copied to dist/icons/"
        elif [ ! -f "$dist_path/icons/eye.svg" ] && [ -f "public/icons/eye.svg" ]; then
            echo_warning "eye.svg missing - copying from public/"
            sudo mkdir -p "$dist_path/icons"
            sudo cp public/icons/eye.svg "$dist_path/icons/"
            sudo chown www-data:www-data "$dist_path/icons/eye.svg"
            sudo chmod 644 "$dist_path/icons/eye.svg"
            echo_success "eye.svg copied"
        else
            echo_success "Icons verified in dist/"
        fi
        echo ""
    fi
    
    # Generate API server block
    api_config="/tmp/nginx-api-${api_domain}.conf"
...
server {
    ...
    location / {
        ...
        # Timeouts for long-lived connections (SignalR)
...
EOF
    ...
    sed -i "s|API_DOMAIN|${api_domain}|g" "$api_config"
    sed -i "s|CERT_PATH|${cert_path}|g" "$api_config"
    sed -i "s|KEY_PATH|${key_path}|g" "$api_config"
    
    echo_success "Generated API server block: ${api_config}"
    
    # Generate UI server block (if different from API domain)
    if [ "$ui_domain" != "$api_domain" ]; then
        ui_config="/tmp/nginx-ui-${ui_domain}.conf"
...
server {
    ...
    ssl_certificate UI_CERT_PATH;
    ssl_certificate_key UI_KEY_PATH;
    ...
    access_log /var/log/nginx/UI_DOMAIN-access.log;
    error_log /var/log/nginx/UI_DOMAIN-error.log;
}
...
EOF
        ...
        # Replace placeholders
        sed -i "s|UI_DOMAIN|${ui_domain}|g" "$ui_config"
        sed -i "s|UI_CERT_PATH|${ui_cert_path}|g" "$ui_config"
        sed -i "s|UI_KEY_PATH|${ui_key_path}|g" "$ui_config"
        sed -i "s|DIST_PATH|${dist_path}|g" "$ui_config"
        
        echo_success "Generated UI server block: ${ui_config}"
    fi
    
    echo ""
    
    # Review and install
    echo_color "=== Review Configuration ===" ${color_blue}
    echo ""
    echo_info "API Server Block:"
    echo "-------------------------------------------"
    cat "$api_config"
    echo "-------------------------------------------"
    echo ""
    ...
    # Install configurations
    echo ""
    echo_color "=== Install Configurations ===" ${color_blue}
    echo ""
    
    sudo cp "$api_config" "/etc/nginx/sites-available/${api_domain}"
    echo_success "Copied API config to /etc/nginx/sites-available/${api_domain}"
    
    sudo ln -sf "/etc/nginx/sites-available/${api_domain}" "/etc/nginx/sites-enabled/"
    echo_success "Enabled site: ${api_domain}"
    
    if [ -n "$ui_config" ]; then
        sudo cp "$ui_config" "/etc/nginx/sites-available/${ui_domain}"
        echo_success "Copied UI config to /etc/nginx/sites-available/${ui_domain}"
        
        sudo ln -sf "/etc/nginx/sites-available/${ui_domain}" "/etc/nginx/sites-enabled/"
        echo_success "Enabled site: ${ui_domain}"
    fi
    ...
    # Test configuration
    echo_color "=== Test Configuration ===" ${color_blue}
    echo ""
    ...
    if sudo nginx -t; then
        ...
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
            if sudo systemctl reload nginx; then
                ...
                echo_error "Failed to reload Nginx"
                exit 1
            fi
        else
            echo_warning "Nginx NOT reloaded - you must reload manually"
            echo_info "Run: sudo systemctl reload nginx"
        fi
    else
    ...
    echo_color "=== Configuration Complete ===" ${color_green}
    echo ""
    echo_info "Nginx has been configured with:"
    echo "  ✓ WebSocket support (Connection upgrade)"
    echo "  ✓ SSL/TLS encryption"
    echo "  ✓ Automatic HTTP → HTTPS redirect"
    echo "  ✓ SPA routing support"
    ...
    echo ""
    echo_info "Monitor logs:"
    echo "  sudo tail -f /var/log/nginx/${api_domain}-error.log"
...
app_deploy_https(){
    ...
    # Ask if user wants to configure reverse proxy automatically
    echo ""
    echo_color "=== Reverse Proxy Configuration ===" ${color_blue}
    echo ""
    echo_info "Would you like to automatically configure a reverse proxy?"
    echo ""
    echo "Select reverse proxy to configure:"
    echo "  1 - Apache (recommended for Ubuntu/Debian)"
    echo "  2 - Nginx (lightweight alternative)"
    echo "  0 - Skip (configure manually)"
    echo ""
    read -p "Enter choice (0/1/2): " -n 1 -r
    echo
    echo ""
    ...
    case $REPLY in
        1)
            ...
            proxy_type="apache"
            echo_info "Will configure Apache after deployment"
            ;;
        2)
            configure_proxy=true
            proxy_type="nginx"
            echo_info "Will configure Nginx after deployment"
            ;;
        0|[Nn])
            configure_proxy=false
            echo_info "Skipping reverse proxy configuration"
            echo_warning "You will need to configure your reverse proxy manually"
            ;;
        *)
            echo_warning "Invalid choice, skipping configuration"
            configure_proxy=false
            ;;
    esac
    echo ""
    ...
    if ./install.sh; then
        ...
        # Configure reverse proxy if requested
        if [ "$configure_proxy" = true ]; then
            ...
            if [ "$proxy_type" = "apache" ]; then
                ...
                echo_color "=== Configuring Nginx ===" ${color_blue}
                echo ""
                configure_nginx
            fi
        fi
        ...
        if [ "$configure_proxy" = true ]; then
            echo_success "${proxy_type^} is configured and ready!"
