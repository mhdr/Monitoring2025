...
const ActiveAlarmsDetailPage: React.FC = () => {
  ...
  const fetchActiveAlarms = useCallback(async (isRefresh = false) => {
    try {
      ...
      // DIFFERENCE: If itemId is in query string, filter to only that item
      let itemIds: string[];
      if (itemId) {
        // Filter to specific itemId from query string
        itemIds = [itemId];
        logger.log('Filtering alarms to specific itemId from query string:', itemId);
      } else {
        // Get all items (main page behavior)
        itemIds = storedItems
          .map(item => item.id)
          .filter((id): id is string => id !== null && id !== undefined);
      }
  ...
  }, [itemId, t, fetchInstantaneousValues, updateActiveAlarms]);
